...
if %errorlevel% neq 0 (
    echo.
    echo [WARN] Ngrok is not installed
    echo.
    ...
    echo   2. Install ngrok now (requires Chocolatey or manual download)
...
start "Cogniclaim Dev Server" cmd /k "title Cogniclaim Dev Server && npm run dev"
...
set /a attempts+=1
if %attempts% geq 15 (
    echo [ERROR] Dev server failed to start after 30 seconds
    echo [INFO] Check if port 5173 is already in use
    echo [INFO] Or check the dev server window for errors
    pause
    exit /b 1
)
echo        Still waiting... (%attempts%/15)
goto :wait_server
...
if %errorlevel% equ 0 (
    ...
    REM Try to get ngrok URL using PowerShell (more reliable)
    for /f "delims=" %%i in ('powershell -Command "try { $tunnels = Invoke-RestMethod -Uri 'http://localhost:4040/api/tunnels' -ErrorAction Stop; if ($tunnels.tunnels.Count -gt 0) { Write-Output $tunnels.tunnels[0].public_url } } catch { }"') do set NGROK_URL=%%i
    
    if defined NGROK_URL (
        echo [OK] Using existing ngrok tunnel: %NGROK_URL%
...
start "Ngrok Tunnel" cmd /k "title Ngrok Tunnel && ngrok http 5173 --domain=india-pikelike-margurite.ngrok-free.dev"
timeout /t 5 /nobreak >nul

REM Try to get ngrok URL
set NGROK_URL=
for /f "delims=" %%i in ('powershell -Command "Start-Sleep -Seconds 3; try { $tunnels = Invoke-RestMethod -Uri 'http://localhost:4040/api/tunnels' -ErrorAction Stop; if ($tunnels.tunnels.Count -gt 0) { Write-Output $tunnels.tunnels[0].public_url } } catch { }"') do set NGROK_URL=%%i
...
if defined NGROK_URL (
    ...
    echo [INFO] Check the Ngrok window for errors
    set FINAL_URL=%DEV_URL%
)
