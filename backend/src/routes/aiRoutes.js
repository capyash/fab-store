...
// Chat agent imported dynamically
 ...
 * 
 * Request body: { message: string, claimId?: string, conversationHistory?: array }
...
router.post('/chat', async (req, res) => {
  try {
    ...
    try {
      // Get claim data if claimId provided
      let claim = null;
      if (claimId) {
        claim = await getClaimById(claimId);
      }
      
      const { sendChatMessage } = await import('../services/ai/chatAgent.js');
