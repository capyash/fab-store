...
function SOPRow({ sop, active, onOpenViewer, claimStatus, scenario, searchQuery ="" }) {
 ...
 const highlightText = (text, query) => {
  ...
  return parts.map((part, i) => 
   part.toLowerCase() === query.toLowerCase() ? (
    <mark key={i} className="bg-alert01 px-0.5 rounded">{part}</mark>
   ) : part
  );
 };
 ...
 return (
  <motion.div
   initial={false}
   animate={{
    borderColor: active ?"pinkTP" : undefined,
    boxShadow: active 
   ...
   className={`rounded-xl border-2 ${
    active 
     ?"border-pinkTP bg-gradient-to-br from-[tertiary] to-[tertiary]/10/10" 
     :"border-stroke01 bg-white hover:border-stroke01:border-text02"
  ...
  >
   <button
    ...
    className={`w-full px-4 py-3.5 flex items-center justify-between transition-colors ${
     active 
      ?"bg-gradient-to-r from-[tertiary]/50 to-transparent hover:from-[tertiary] hover:to-[tertiary]" 
      :"hover:bg-bg02:bg-text01"
    }`}
   >
    <div className="flex items-center gap-3 flex-1 min-w-0">
     <motion.span
      ...
      className={`text-xs font-mono px-2.5 py-1 rounded-md font-semibold shrink-0 ${
       active
        ?"bg-pinkTP text-white shadow-md"
        :"bg-bg03 text-text01"
      }`}
     >
      {sop.id}
     ...
     <div className="flex-1 min-w-0">
      <div className={`text-sm font-medium truncate ${active ?"text-pinkTP" :"text-text01"}`}>
       {highlightText(sop.title, searchQuery)}
      </div>
      {sop.page && (
       <div className="text-xs text-text03 mt-0.5">
        {sop.page}
       </div>
      )}
     </div>
    ...
    <ChevronDown 
     className={`w-4 h-4 transition-transform duration-200 shrink-0 ml-2 ${
      open ?"rotate-180" :""
     } ${active ?"text-pinkTP" :"text-text03"}`} 
    />
   </button>
     ...
     >
      <div className={`px-4 pb-4 pt-2 space-y-3 ${
       ...
       <button
        ...
        className="w-full flex items-center justify-center gap-2 px-3 py-2.5 text-xs font-semibold bg-gradient-to-r from-pinkTP to-pinkTP text-white rounded-lg hover:from-pinkTP hover:to-pinkTP transition-all shadow-md hover:shadow-lg"
       >
       ...
       {sop.text && sop.text.includes('\n\n') && (
        <div className="pt-2 border-t border-stroke01">
         <p className="text-xs font-semibold text-text01 mb-2">Quick Steps:</p>
         <div className="flex flex-wrap gap-1.5">
          {sop.text.split('\n\n').map((step, idx) => (
           <button
            ...
            className="px-2.5 py-1.5 text-xs bg-bg03 text-text01 rounded-md hover:bg-stroke01:bg-text01 transition-colors font-medium"
           >
            Step {idx + 1}
           </button>
          ))}
 ...
 return (
  <>
   ...
   <motion.aside
    ...
    className="relative bg-white border-l border-stroke01 shadow-xl flex flex-col h-full shrink-0"
    style={{ width: `${width}px`, minWidth: `${width}px` }}
   >
    {/* Header */}
    <div className="bg-gradient-to-r from-pinkTP to-pinkTP text-white px-4 py-3.5 flex items-center justify-between shrink-0">
    ...
    {/* Search Bar */}
    <div className="px-4 py-3 border-b border-stroke01 shrink-0">
     <div className="relative">
      ...
      <input
       ...
       className="w-full pl-9 pr-3 py-2 text-sm border border-stroke01 rounded-lg bg-white text-text01 placeholder-text03 focus:outline-none focus:ring-2 focus:ring-pinkTP/50"
      />
     </div>
    </div>
    ...
    <div
     ...
     className="absolute left-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-pinkTP/30 transition-colors group"
     title="Drag to resize"
    >
     <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 w-1 h-12 bg-pinkTP rounded-full opacity-0 group-hover:opacity-100 transition-opacity" />
    </div>
   </motion.aside>
