...
const Field = ({ k, v }) => (
 <div>
  <div className="text-xs text-text03 uppercase tracking-wide mb-0.5">{k}</div>
  <div className="text-sm font-medium text-text01">{v ||"N/A"}</div>
 </div>
);

function SLATracker({ deadline, daysUntilSLA }) {
 const getColor = () => {
  if (daysUntilSLA === undefined || daysUntilSLA === null) return"text-text02";
  if (daysUntilSLA < 0) return"text-error03";
  if (daysUntilSLA < 7) return"text-pinkTP";
  if (daysUntilSLA < 14) return"text-alert02";
...
export default function LoanDetailsPanel({ loanData, onClose }) {
 ...
 return (
  <div className="bg-white border-l border-stroke01 w-96 flex flex-col h-full">
   <div className="flex items-center justify-between p-4 border-b border-stroke01">
    <h3 className="text-lg font-semibold text-text01">Loan Details</h3>
    <button
     onClick={onClose}
     className="text-text03 hover:text-text02:text-stroke01"
    >
