...
import { aiAPI, loansAPI } from"../services/api";
...
/* -------------------- MAIN COMPONENT -------------------- */
export default function UnifiedAIConsole({ onBind, onSOPReference, onSOPView, loanId, loan: loanData = null }) {
 ...
 // Initialize AI reasoning when loan is loaded
 useEffect(() => {
  if (!reasoningInitialized.current && loanId && loanData) {
   reasoningInitialized.current = true;
   setThinking(true);
   
   const executeAIAnalysis = async () => {
    try {
     let loanObj = loanData;
     if (!loanObj && loanId) {
      loanObj = await loansAPI.getById(loanId);
     }
     ...
     aiAPI.analyzeLoan(loanObj, onStep)
      .then((finalResult) => {
       setThinking(false);
       setStreamingMessage(null);
       setCurrentStep(null);
 ...
 useEffect(() => {
  ...
  onBind?.(api);
 }, [onBind, onSOPReference, pushMessage, loanData, loanId, messages]);
 ...
 const handleAction = useCallback((actionType, recommendation) => {
  ...
  setLastAction({
   ...
   message:
    actionType ==="approve"
     ?"This would approve the loan in a real environment (no changes made in demo mode)."
     : actionType ==="deny"
     ?"This would deny the loan in a real environment (no changes made in demo mode)."
 ...
 return (
  <div className="flex flex-col h-full bg-bg02">
   ...
   <div className="flex-1 overflow-y-auto px-4 sm:px-6">
    ...
    {(engineMessages.length > 0 || thinking) && (
     ...
     <div className="relative flex items-center gap-3 pb-3 mb-2">
      <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-pinkTP to-pinkTP rounded-full"></div>
      
      <div className="p-2 rounded-lg bg-gradient-to-br from-pinkTP to-pinkTP shadow-lg shrink-0">
       <Brain className="w-5 h-5 text-white" />
      </div>
      <div className="flex-1 min-w-0">
       <h2 className="text-xl font-bold bg-gradient-to-r from-pinkTP to-pinkTP bg-clip-text text-transparent">
        AI Reasoning
       </h2>
       <p className="text-xs text-text02 mt-0.5">
        Multi-agent reasoning console powered by SOP Executor
       </p>
      </div>
      {avgConfidence && (
       <div className="px-3 py-1.5 rounded-lg bg-gradient-to-r from-pinkTP/10 to-pinkTP/20/20 border border-pinkTP/20 shrink-0">
        <span className="text-xs font-semibold text-pinkTP">
     ...
     {thinking && stepsOnly.length === 0 && (
      <motion.div
       initial={{ opacity: 0, y: 10 }}
       animate={{ opacity: 1, y: 0 }}
       className="bg-gradient-to-r from-pinkTP/10 to-pinkTP/20/20 border border-pinkTP/20 rounded-xl p-5 shadow-lg backdrop-blur-sm"
      >
       <div className="flex items-center gap-4">
        <motion.div
         className="p-3 rounded-xl bg-gradient-to-br from-pinkTP to-pinkTP shadow-lg"
         animate={{ 
          rotate: [0, 360],
          scale: [1, 1.1, 1]
         }}
        ...
        <div className="flex-1">
         <h3 className="text-lg font-bold text-pinkTP mb-1">AI Analysis in Progress</h3>
         <p className="text-sm text-text02">TP Lend is analyzing the loan using multi-agent reasoning...</p>
          ...
          <div className="mt-3 flex items-center gap-3">
           <div className="flex-1 h-2 bg-stroke01 rounded-full overflow-hidden">
            <motion.div
             ...
             className="h-full bg-gradient-to-r from-pinkTP to-pinkTP rounded-full"
            />
           </div>
           <span className="text-xs font-semibold text-pinkTP">{Math.round(reasoningProgress)}%</span>
          </div>
         )}
        </div>
       </div>
  ...
  {/* Fixed Chat Input at Bottom */}
   <div className="flex-shrink-0 bg-white border-t border-stroke01 px-6 py-4 shadow-lg">
    <div className="max-w-[1600px] mx-auto">
     <div className="flex items-center gap-3 mb-2">
      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-pinkTP to-pinkTP flex items-center justify-center flex-shrink-0">
       <Brain className="w-5 h-5 text-white" />
      </div>
      <div className="flex-1 relative">
       <input
        ref={chatInputRef}
        type="text"
        placeholder="Ask TP Lend about this loan..."
        className="w-full px-4 py-3 rounded-lg border border-stroke01 bg-white text-text01 placeholder-text03 focus:outline-none focus:ring-2 focus:ring-pinkTP/50 focus:border-transparent"
        onKeyDown={(e) => {
      ...
      <button
       ...
       className="p-3 rounded-lg bg-gradient-to-r from-pinkTP to-pinkTP text-white hover:shadow-lg transition-all hover:scale-105 active:scale-95 flex-shrink-0"
       title="Send message"
      >
     ...
     <p className="text-xs text-text03 ml-[58px]">Press Enter to send</p>
    </div>
   </div>
   ...
   {lastAction && (
    ...
    >
     <div className="rounded-xl border border-stroke01 bg-white shadow-xl px-4 py-3 flex items-start gap-3">
      <div className="mt-1">
      ...
      <div className="flex-1">
       <div className="flex items-center justify-between gap-3 mb-1">
        ...
        <button
         ...
         className="text-xs text-text03 hover:text-text01:text-stroke01"
        >
         Close
        </button>
       </div>
