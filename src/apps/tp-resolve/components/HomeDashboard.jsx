 ...
 const statCards = [
  {
   ...
   icon: FileText,
   gradient:"from-pinkTP to-pinkTP",
  },
  {
   title:"Appeals",
   value: stats.appeals,
  ...
  },
  {
   ...
   icon: Activity,
   gradient:"from-alert010 to-error010",
  },
  {
   title:"Urgent Deadlines",
   value: stats.urgentDeadlines,
   icon: Clock,
   gradient:"from-error010 to-alert010",
  },
  {
   title:"Resolved",
 ...
 if (loading) {
  return (
   <div className="flex items-center justify-center h-64">
    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-pinkTP"></div>
   </div>
  );
 }
   ...
   <div className="bg-white rounded-2xl border border-stroke01 shadow-sm p-6 md:p-7">
    ...
    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
     <div className="flex items-center gap-3">
      <div className="p-3 rounded-xl bg-gradient-to-br from-pinkTP via-pinkTP to-pinkTP shadow-lg">
     ...
     >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
       <div className="bg-white/80 rounded-lg p-4 border border-error01">
        <div className="text-xs text-text02 mb-1">Inbox</div>
        <div className="text-2xl font-bold text-error03 mb-1">
         {stats.underInvestigation}
        </div>
        <div className="text-xs text-text03">high-touch cases</div>
       </div>
       <div className="bg-white/80 rounded-lg p-4 border border-error01">
        <div className="text-xs text-text02 mb-1">Cases Near SLA (7 days)</div>
        <div className="text-2xl font-bold text-error03 mb-1">
         {stats.urgentDeadlines}
        </div>
        <div className="text-xs text-text03">appeals &amp; grievances</div>
       </div>
       <div className="bg-white/80 rounded-lg p-4 border border-alert01">
        <div className="text-xs text-text02 mb-1">Open Appeals (Total)</div>
        <div className="text-2xl font-bold text-pinkTP mb-1">
         {stats.appeals}
        </div>
        <div className="text-xs text-text03">in queue</div>
       </div>
       <div className="bg-white/80 rounded-lg p-4 border border-alert01">
        <div className="text-xs text-text02 mb-1">Total Disputed Amount</div>
        <div className="text-2xl font-bold text-pinkTP mb-1">
         ${((stats.totalAmount || 0) / 1000000).toFixed(1)}M
        </div>
        <div className="text-xs text-text03">exposure in current docket</div>
       </div>
      </div>
     </motion.div>
    </div>
    ...
    <div className="mb-8">
     <div className="flex items-center gap-2 mb-4">
      <div className="p-1.5 rounded-lg bg-gradient-to-br from-neutral010/20 to-pinkTP/100/20 border border-neutral010/30">
       <Activity className="w-4 h-4 text-primary" />
      </div>
      <h3 className="text-lg font-semibold text-text01">
       AI-Powered Priority Queue
      </h3>
      <span className="text-xs px-2 py-1 rounded-full bg-neutral01 text-textLink font-medium">
       🤖 AI Recommended
      </span>
     </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
       {priorityCases.map((c, idx) => (
        <motion.div
         ...
         className="rounded-xl border-2 bg-gradient-to-br from-neutral01 to-pinkTP/10 border-neutral02/40 bg-white/90 backdrop-blur p-4 hover:shadow-xl transition-all hover:scale-[1.02] cursor-pointer"
        >
         ...
         <div className="mt-2 pt-2 border-t border-neutral02/40">
    ...
    <div className="mb-8">
     <div className="flex items-center gap-2 mb-4">
      <div className="p-1.5 rounded-lg bg-gradient-to-br from-pinkTP/100/20 to-pinkTP/100/20 border border-pinkTP/100/30">
       <AlertCircle className="w-4 h-4 text-pinkTP" />
      </div>
      <h3 className="text-lg font-semibold text-text01">
       Anomaly Detection
      </h3>
      <span className="text-xs px-2 py-1 rounded-full bg-pinkTP/20 text-textLink font-medium">
       🤖 AI Detected
      </span>
     </div>
     {anomalies.length > 0 ? (
       ...
       {anomalies.map((a, idx) => (
        <motion.div
         ...
         className={`rounded-xl border-2 bg-gradient-to-br ${
          a.severity ==="high"
           ?"from-error010/20 to-alert010/20 border-error010/30"
           :"from-alert010/20 to-alert010/20 border-alert010/30"
    ...
    <div className="mt-2 border-t border-bg03 pt-5">
     ...
     <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      <button
       ...
       className="p-4 border border-stroke01 rounded-lg hover:border-pinkTP hover:bg-pinkTP/30 transition-all text-left"
      >
       <div className="font-semibold text-text01 mb-1 flex items-center gap-2">
        New Appeal
        <ArrowRight className="w-3 h-3 text-pinkTP" />
       </div>
