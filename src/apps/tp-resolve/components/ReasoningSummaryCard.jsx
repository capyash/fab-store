 ...
 * Adapted for TP Resolve (cases instead of claims)
 */
...
export default function ReasoningSummaryCard({ recommendation, steps = [], case: caseData, onSOPView, onAction }) {
  ...
  // Derive primary SOP similar to Cogniclaim: scenario first, then status
  const primaryScenarioSOP =
    caseData?.scenario && SCENARIO_SOPS[caseData.scenario]
      ? SCENARIO_SOPS[caseData.scenario]
      : null;
  const statusKey =
    caseData?.status && SOP_INDEX[caseData.status] ? caseData.status : null;
  const primaryStatusSOP = statusKey ? SOP_INDEX[statusKey] : null;
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {recommendation.sopRefs.map((ref, idx) => (
                        <motion.button
                          key={idx}
                          onClick={() => onSOPView?.(ref, null, caseData?.status, null)}
